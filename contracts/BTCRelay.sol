...
contract BTCRelay {
     ...
     */
    event ChainReorg(
        bytes32 indexed newChainTip,
        uint256 indexed startHeight,
        uint256 indexed forkId
    );

    // EXCEPTION MESSAGES
    string ERR_GENESIS_SET = "Initial parent has already been set";
    string ERR_INVALID_FORK_ID =
        "Incorrect fork identifier: id 0 is no available";
    ...
    string ERR_DIFF_TARGET_HEADER =
        "Incorrect difficulty target specified in block header";
    string ERR_NOT_MAIN_CHAIN =
        "Main chain submission indicated, but submitted block is on a fork";
    string ERR_FORK_PREV_BLOCK =
        "Previous block hash does not match last block in fork submission";
    string ERR_NOT_FORK =
        "Indicated fork submission, but block is in main chain";
    ...
    function setInitialParent(
        ...
        _highScoreAndBlockHeight =
            (uint256(blockHeight) << 224) |
            (chainWork &
        ...
        bytes32 keccak256thisBlockHeader = storeBlockHeader(
            blockHeight,
            0,
            storedHeader
        );
        assembly {
            log3(
                storedHeader,
                160,
                0x9fc4fb2e64c90ee101e27a74385448b64fa038e3075908ea993a337abecfb242,
                keccak256thisBlockHeader,
                blockHeaderHash
            )
        }
    }
     ...
     */
    function submitMainChainHeaders(
        bytes calldata blockHeaderBytes,
        HeaderInfo calldata prevBlockHeader
    ) public {
     ...
     */
    function submitNewForkChainHeaders(
        bytes calldata blockHeaderBytes,
        HeaderInfo calldata prevBlockHeader
    ) public returns (uint256 forkCounter) {
     ...
     */
    function submitForkChainHeaders(
        bytes calldata blockHeaderBytes,
        uint256 forkId,
        HeaderInfo calldata prevBlockHeader
    ) public {
    ...
    function submitBlockHeaders(
        ...
        uint256 forkId,
        HeaderInfo memory storedHeader
    ) private {
...

        {
            ...
            if (forkId == 0) {
                require(
                    getCommitment(blockHeight, blockHeight, startHeight) ==
                        keccak256prevBlockHeader,
                    "Invalid prev block commitment"
                );
            } else {
                ...
                if (_ongoingForks[forkId].forkHeaderHashes.length != 0) {
                    //Existing fork
                    require(
                        getLatestForkHash(forkId) == keccak256prevBlockHeader,
                        ERR_FORK_PREV_BLOCK
                    );
                } else {
                    //New fork
                    ...
                    require(
                        getCommitment(blockHeight, maxHeight, startHeight) ==
                            keccak256prevBlockHeader,
                        "Invalid prev block commitment"
                    );
                }
        ...
        for (; i < end; i += 80) {
...

            {
...

                {
                    ...
                    require(
                        reverse(hashCurrentBlock) <= bytes32(target),
                        ERR_LOW_DIFF
                    );
                ...
                require(
                    largerThanMedian(data1, data2, ts),
                    "Block timestamp too low"
                );
                require(
                    ts < block.timestamp + MAX_FUTURE_BLOCKTIME,
                    "Block timestamp too high"
                );

                data1 =
                    ((data1 << 32) &
                        0x0000000000000000000000000000000000000000FFFFFFFFFFFFFFFF00000000) |
            ...
            if (forkId == 0) {
                ...
                if (i == 0) {
                    require(
                        storedHeader.chainWork > highScore,
                        ERR_NOT_MAIN_CHAIN
                    );
                }
                startHeight = storeBlockCommitment(
                    blockHeight,
                    startHeight,
                    keccak256thisBlockHeader
                );
                ...
                if (_ongoingForks[forkId].forkHeaderHashes.length != 0) {
                    ...
                    require(
                        getCommitment(blockHeight) != keccak256thisBlockHeader,
                        ERR_NOT_FORK
                    );
                ...
                assembly {
                    log4(
                        storedHeader,
                        160,
                        0x2d8d54f6e05054b3febd04b2067c23c239093d4bb4e053a474ea38e46e79c283,
                        keccak256thisBlockHeader,
                        lastBlockHash,
                        forkId
                    )
        ...
        if (forkId == 0) {
            ...
            _highScoreAndBlockHeight =
                (uint256(blockHeight) << 224) |
                (storedHeader.chainWork &
                    0x00000000FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF);
            if (uint32(lastDiffAdjustmentTime) != _lastDiffAdjustmentTime)
                _lastDiffAdjustmentTime = uint32(lastDiffAdjustmentTime);
        }
    }

    function executeForkAndRemove(
        uint256 forkId,
        uint256 startHeight
        ...
        for (uint e = 0; e < forkLen; e++) {
            ...
            startHeight = storeBlockCommitment(
                currentHeight,
                startHeight,
                _ongoingForks[forkId].forkHeaderHashes[e]
            );
            //_mainChain[currentHeight] = _ongoingForks[forkId].forkHeaderHashes[e];
            currentHeight++;
        }
        emit ChainReorg(
            getCommitment(currentHeight - 1),
            forkStartHeight,
            forkId
        );
    ...
    function storeBlockHeader(
        ...
        uint256 startHeight,
        HeaderInfo memory storedHeader
    ) private returns (bytes32 commitmentHash) {
        assembly {
    ...
    function verifyTX(
        ...
        require(
            getCommitment(txBlockHeight, blockHeight, startHeight) ==
                commitmentHash,
            "Invalid block header supplied"
        );
        //bytes32 merkleRoot =_headers[blockHeaderHash].merkleRoot;

        // compute merkle tree root and check if it matches block's original merkle tree root
        if (
            computeMerkle_calldata(reversedTxid, txIndex, merkleProof) ==
            blockHeader.merkleRoot
        ) {
            return true;
        }
    ...
    function dblSha_memory(
        HeaderInfo memory data
    ) private view returns (bytes32 value) {
        /// @solidity memory-safe-assembly
        assembly {
            function reverseUint32(input) -> output {
                output := or(
                    shr(8, and(input, 0xFF00FF00)),
                    shl(8, and(input, 0x00FF00FF))
                )
                output := or(
                    shr(16, and(output, 0xFFFF0000)),
                    shl(16, and(output, 0x0000FFFF))
                )
            }
            ...
            mstore(
                add(memPtr, 68),
                shl(224, reverseUint32(mload(add(data, 128))))
            ) //Timestamp
            mstore(add(memPtr, 72), mload(add(data, 100))) //nbits
            mstore(add(memPtr, 76), mload(add(data, 104))) //nonce
    ...
    function reverseUint32(uint256 output) private pure returns (uint256) {
        assembly {
            // swap bytes
            output := or(
                shr(8, and(output, 0xFF00FF00)),
                shl(8, and(output, 0x00FF00FF))
            )
            output := or(
                shr(16, and(output, 0xFFFF0000)),
                shl(16, and(output, 0x0000FFFF))
     ...
     */
    function targetTonBits(
        uint256 target
    ) private pure returns (uint256 nBits) {
        assembly {
            let start := 248
            for {

            } and(gt(start, 0), iszero(and(target, shl(start, 0xFF)))) {
                start := sub(start, 8)
            } {
            ...
            if not(gt(nSize, 3)) {
                nCompact := shl(mul(8, sub(3, nSize)), target)
            }

            if gt(nSize, 3) {
                nCompact := shr(mul(8, sub(nSize, 3)), target)
            }
            ...
            nCompact := or(
                ...
                or(and(shr(8, nCompact), 0xFF00), nSize)
            )
    ...
    function nBitsToTarget(uint256 nBits) private pure returns (uint256) {
        ...
        assembly {
            ...
            let nWord := or(
                or(and(shl(8, nBits), 0x7F0000), and(shr(8, nBits), 0xFF00)),
     ...
     */
    function difficultyShouldBeAdjusted(
        uint256 blockHeight
     ...
     */
    function correctDifficultyTarget(
        HeaderInfo memory prevBlockHeader,
        uint256 blockHeight,
        uint256 nBits
    ...
    function correctDifficultyTarget(
        ...
        //     // Difficulty not adjusted at this block blockHeight
        //     require(
        //         !(nBits != prevBlockHeader.nbits && prevTarget != 0),
        //         ERR_DIFF_TARGET_HEADER
        //     );
        //     return false;
        // } else {
    ...
    function computeMerkle_calldata(
        bytes32 reversedTxHash,
        uint256 txIndex,
        bytes calldata reversedMerkleProof
    ) private view returns (bytes32 value) {
        //  Special case: only coinbase tx in block. Root == proof
        if (reversedMerkleProof.length == 0) return reversedTxHash;
            ...
            for {
                ...
                pop(
                    staticcall(
                        ...
                        0x00,
                        0x20,
                        mul(and(txIndex, 0x1), 0x20),
                        32
                    )
                ) //goes to first position if txIndex & 1 == 1 else second position
            }
    ...
    function getLatestForkHash(uint256 forkId) public view returns (bytes32) {
        return
            _ongoingForks[forkId].forkHeaderHashes[
                _ongoingForks[forkId].forkHeaderHashes.length - 1
            ];
    }
}
