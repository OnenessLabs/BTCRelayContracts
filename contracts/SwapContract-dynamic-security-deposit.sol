...
contract CrossLightningSwaps {
    ...
    */
    ...
    function offerer_claimInit(
        ...
        require(
            payReq.claimerBounty == 0 && payReq.securityDeposit == 0,
            "Invalid deposit"
        );
        ...
        bytes32 commitment;
        assembly {
            commitment := keccak256(payReq, 256)
        }
    ...
    function offerer_init(
        ...
        require(
            msg.value >= payReq.claimerBounty &&
                msg.value >= payReq.securityDeposit,
    ...
    function payOutClaimerBounty(
        AtomicSwapStruct memory payReq,
        address initiator
    ) internal {
        uint256 securityDeposit = payReq.securityDeposit;
        uint256 claimerBounty = payReq.claimerBounty;
        if (claimerBounty > 0) {
            payable(initiator).transfer(securityDeposit);
            if (securityDeposit <= claimerBounty) return;
            securityDeposit -= claimerBounty;
        }
        if (securityDeposit > 0)
            payable(payReq.claimer).transfer(securityDeposit);
    }

    function payOutSecurityDeposit(AtomicSwapStruct memory payReq) internal {
        uint256 securityDeposit = payReq.securityDeposit;
        uint256 claimerBounty = payReq.claimerBounty;
        if (securityDeposit > 0) {
            payable(payReq.offerer).transfer(securityDeposit);
            if (claimerBounty <= securityDeposit) return;
            claimerBounty -= securityDeposit;
        }
        if (claimerBounty > 0) payable(payReq.claimer).transfer(claimerBounty);
    }

    function payOutSecurityDepositClaimerOnly(
        AtomicSwapStruct memory payReq
    ) internal {
        uint256 securityDeposit = payReq.securityDeposit;
        uint256 claimerBounty = payReq.claimerBounty;
    ...
    function _refundSingleNoSendNoRevert(
        ...
        if (((payReq.data >> 152) & 0xFF) > 0) {
            ...
            payOutSecurityDepositClaimerOnly(payReq);
        }
    ...
    function offerer_multi_refund(
        ...
        for (uint i = 0; i < payReqs.length; i++) {
            ...
            if (!isPayIn && amountRefunded > 0)
                securityDeposits += payReqs[i].securityDeposit;
        ...
        if (securityDeposits > 0)
            payable(msg.sender).transfer(securityDeposits);
    }
    ...
    function claimer_claim_extAddress(
        AtomicSwapStruct memory payReq,
        bytes32 secret,
        address claimerBountyAddress
    ) public {
        uint256 expiry = payReq.data & 0xFFFFFFFFFFFFFFFF;
    ...
    function claimer_claimWithTxData(
        ...
        claimer_claimWithTxData_extAddress(
            payReq,
            vout,
            txData,
            proof,
    ...
    function claimer_claimWithTxData_extAddress(
        AtomicSwapStruct memory payReq,
        uint256 vout,
        bytes memory txData,
        TransactionProof calldata proof,
